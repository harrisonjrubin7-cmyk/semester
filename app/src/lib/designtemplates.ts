...
const words = (
  ...
  bold = true,
  opacity = 1,
): Omit<DesignLayer, 'id' | 'fileId'> => ({ kind: 'text', text, fill, fontSize, bold, opacity, rotation: 0, gradient: null, chartKind: 'column', ...at });
...
const block = (
  ...
  opacity = 1,
): Omit<DesignLayer, 'id' | 'fileId'> => ({ kind, text: '', fill, fontSize: 48, bold: false, opacity, rotation: 0, gradient: null, chartKind: 'column', ...at });
