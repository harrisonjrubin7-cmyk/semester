...
const SETTINGS: { name: string; options: { id: string; label: string }[]; does: string; doesNot: string }[] = [
  ...
  { name: 'Navigation', options: NAVS, does: 'Which single navigation the app draws: the workspace — tabs across the top and one search bar under them — the browser shell, a tab bar, one feed, a home screen of icons, two rows of shelves, or your study guides. On Layout and navigation.', doesNot: 'It never draws two. Every screen is reachable in all seven, and search finds them all.' },
