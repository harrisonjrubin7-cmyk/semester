...
export function carryKey<T>(
  e: React.KeyboardEvent,
  carrying: Carrying<T> | null,
  payload: T,
  {
    onPickUp,
    onMove,
    ...
    onPickUp: (c: Carrying<T>) => void;
    onMove: (c: Carrying<T>) => void;
    onDrop: (c: Carrying<T>) => void;
    onCancel: () => void;
    /** Whether up and down mean minutes here, or a week's worth of days. */
    hasTime?: boolean;
  },
): boolean {
  if (!carrying) {
    if (e.key === ' ' || e.key === 'Enter') {
      onPickUp({ payload, days: 0, minutes: 0 });
      return true;
    }
    return false;
  }
  ...
  switch (e.key) {
    ...
    case ' ':
      ...
      return true;
    case 'ArrowLeft':
      onMove({ ...carrying, days: carrying.days - 1 });
      return true;
    case 'ArrowRight':
      onMove({ ...carrying, days: carrying.days + 1 });
      return true;
    case 'ArrowUp':
      onMove(
        hasTime
          ? { ...carrying, minutes: carrying.minutes - (e.shiftKey ? STEP_MINUTES : 60) }
          : { ...carrying, days: carrying.days - 7 },
      );
      return true;
    case 'ArrowDown':
      onMove(
