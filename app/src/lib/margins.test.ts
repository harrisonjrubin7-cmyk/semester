...
describe('a style object with the shorthand expanded', () => {
  ...
  it('keeps a numeric margin a number, so React still reads it as pixels', () => {
    // `screens/Guide.tsx` writes `<SectionLabel style={{ margin: 0 }}>`, which
    // is a shorthand as much as the string form and had the same bug. It must
    // not be stringified: `margin: 8` would become the invalid `margin-top: 8`.
    expect(longhandMargins({ margin: 0 })).toEqual({
      marginTop: 0,
      marginRight: 0,
      marginBottom: 0,
      marginLeft: 0,
    });
    expect(longhandMargins({ margin: 8, marginBottom: 2 })).toEqual({
      marginTop: 8,
      marginRight: 8,
      marginBottom: 2,
      marginLeft: 8,
