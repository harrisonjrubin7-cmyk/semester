...
export function assemble(parsed: Reply, item: Intake, base: Omit<Where, 'page'>): Harvest {
  ...
  for (const raw of [...(parsed.items ?? []), ...(parsed.item ? [parsed.item] : [])]) {
    ...
    // The comment above says "the same validation the syllabus importer
    // makes" — which stopped being true when that one moved to `realMonthDay`
    // and this copy of the sentence was left naming a validation that had
