...
/*
 * The Tools tab became a home screen, and its four are folded into the
 * measurement above rather than added to it: thirteen cards drawn by hand in
 * `screens/Study.tsx` — each with its own padding, its own 13.5px blurb and
 * its own 1.35 leading — became one `<AppGrid>` whose sizes live in `app.css`
 * on the scales. The one value the grid does set by hand, the 11.5px icon
 * name, is inside the numbers. Same reason as the note below: two ratchets
 * from one base do not add up, so 674/220/516/502 is what `counts()` reports
 * on the merged tree — one below main's on every axis, which is this
 * branch's own contribution and no more.
 *
