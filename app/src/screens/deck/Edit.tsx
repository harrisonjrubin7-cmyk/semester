...
export function DeckEdit({ deck }: { deck: StoredDeck }) {
  ...
  /** A new slide of one layout, after the one open. */
    ...
    },
    {
      ...
      groups: [
        LAYOUTS.map((layout) => ({
          ...
          run: () => addSlide(layout.id as Layout),
        })),
      ],
    },
    {
        ...
        tools={
          <>
            ...
            {LAYOUTS.map((layout) => (
              <Tool
                key={layout.id}
                label={`New ${layout.label.toLowerCase()} slide`}
                icon={
                  <>
                    <Plus size={13} />
                    {layout.label}
                ...
                onClick={() => addSlide(layout.id as Layout)}
              />
            ))}
            <ToolRule />
            <Tool
              label="Duplicate this slide"
              disabled={!slide}
              onClick={() => patch(duplicate(deck, on))}
            />
            <Tool
              label={slide && !shown(deck, on) ? 'Bring this slide back' : 'Skip this slide'}
              pressed={slide ? !shown(deck, on) : undefined}
              disabled={!slide}
              onClick={() => patch(toggleHidden(deck, on))}
            />
          </>
        }
      />
