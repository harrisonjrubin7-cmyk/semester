...
function TodayFeed() {
  ...
  if (order.length === 0) {
    return (
      <div style={{ fontSize: 'var(--type-base)', opacity: 'var(--app-text-dim)', padding: '20px 0', lineHeight: 'var(--leading-relaxed)' }}>
          ...
          {untimed.map((t) => (
            ...
            >
              ...
              <span style={{ flex: 1, minWidth: 0, fontSize: 'var(--type-base)' }}>
                {t.title}
                {t.time.trim() && (
                  <span style={{ opacity: 'var(--app-text-dim)' }}> · {t.time.trim()}</span>
                )}
              </span>
            </button>
          ))}
