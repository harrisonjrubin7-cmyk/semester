...
export function SayIt({
  ...
  if (revealed) {
    ...
    return (
      <Panel style={{ marginTop: 'var(--sp-6)' }}>
        ...
        <div style={{ fontSize: 'calc(13.5px * var(--text-scale, 1))', marginTop: 'calc(5px * var(--density, 1))', lineHeight: 'var(--leading-relaxed)', textWrap: 'pretty' }}>
          {said}
        </div>
  ...
  return (
    <div style={{ marginTop: 'var(--sp-6)' }}>
      ...
      {error ? (
        <div role="alert" style={{ fontSize: 'var(--type-sm)', color: 'var(--app-dim)', marginTop: 'var(--sp-4)', textAlign: 'center' }}>{error}</div>
      ) : null}
    </div>
  );
}
