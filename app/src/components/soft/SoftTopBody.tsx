 ...
 * back.
 *
 * ## Why this is a separate file from `SoftTop.tsx`
 *
 * Because it only runs on one of the three shells, and it was costing the
 * other two. `App.tsx` renders `<SoftTop />` in all three, and this file used
 * to be that component — so `lib/softtop.ts` and the eleven modules behind it
 * were parsed before the first render whether or not the Soft shell was on:
 * 3,887 lines, measured on the eager import graph.
 *
 * Worse than the bytes: `useTop()` is a hook, so it ran *before* the
 * `if (!soft) return null` that used to sit under it. Every render on every
 * shell walked the term's deadlines to build a spec that was then thrown
 * away.
 *
 * `SoftTop.tsx` is now the gate — a dozen lines that ask which shell is on
 * and fetch this file when the answer is Soft. Nothing else about the hero
 * changed. See `ENGINEERING-AUDIT.md` §1, P1d.
 *
 ...
 */
function useTop() {
  const { state, catalog, school, sync, account } = useStore();
  const now = useNow();
