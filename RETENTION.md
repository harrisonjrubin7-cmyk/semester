...
| `lti_role_map` | **kept until a migration changes it** | which LTI launch roles become which Semester roles, and one line each on why that one is trusted. Four rows, the same for every account, and no row about a person — so an account deletion has nothing here to reach. It is a trust decision rather than configuration, which is the reason it is a table at all: `Administrator` and `Mentor` are deliberately **absent**, and an absence that lives in a table is one a reviewer can see and a suite can assert, where an absence in a `case` expression is one somebody adds a branch to. Written only by migration; readable by any signed-in account, because every row of it is printed in `docs/ROLE_REQUIREMENTS.md` anyway |
