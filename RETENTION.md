...
| `family_invites` | account deletion of the student, or 14 days unclaimed | the share code a student generated and what it is worth once claimed. Two clocks, deliberately: the **code** lapses in fourteen days because an unclaimed one is a bearer token sitting in somebody's messages, and the **grant** it becomes carries its own expiry, set when the code was made. A claimed row is kept rather than deleted — it is the record of where a grant came from, and the student's screen reads it to say which code a person arrived on. Readable by the student who made it and by nobody else, including whoever holds the code |
