...
| --- | --- | --- | --- |
| **`SUPABASE_SERVICE_ROLE_KEY`** | Injected by Supabase into every function, and exported by hand for the one command that writes `app_admins` (`app/scripts/grant-admin.ts` — see SETUP.md) | **Everything.** It bypasses row-level security: every row of every table, for every account | Dashboard → Settings → API → roll the key. Functions pick the new one up on their next invocation; nothing is redeployed |
