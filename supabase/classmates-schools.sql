...
-- `private`, not `public`, and this file was written before that was true.
--
-- The helper moved schemas after this branch was cut: in `public` it was also
-- a URL, because PostgREST publishes what anon or authenticated may execute
-- there. Left as `create or replace function public.verified_student()`, this
-- migration would have made a *second* copy in the exposed schema — a
-- resurrected `/rest/v1/rpc/verified_student` that no policy reads, since all
-- seven now point at `private`. Dead code whose only effect is an endpoint.
--
-- The rest matches the hardening it has to live alongside: an empty
-- search_path with `auth.users` qualified, `auth.uid()` hoisted so the planner
-- evaluates it once, and EXECUTE to both roles because the policies that call
-- it are `TO public`.
