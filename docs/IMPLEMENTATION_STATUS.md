...
- **Design:** editable text/shapes/images across five layer kinds (text, rectangle, ellipse, triangle, image), mouse and keyboard positioning, per-layer opacity, alignment against the page on both axes, layer duplication and ordering, named page-size presets alongside manual sizing, colors, undo/redo, eight starting templates, SVG/PNG/JPG export and image exports saved in Files. This is still a basic graphic editor and not Canva feature parity: no background removal, charts, free rotation, gradients or comments.
