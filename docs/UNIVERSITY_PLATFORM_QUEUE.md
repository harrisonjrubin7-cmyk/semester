...
| **157 Public university mode** `BUILT` | `PUBLIC_FEATURES` in `app/src/lib/readiness.ts` — profile, course catalogue, organisations, events, campus resources and the academic calendar, all things a university already publishes, all available with no agreement. Refusing a student their own university's published catalogue because Semester has no contract would punish them for a commercial fact. **The item's "only where legally and technically appropriate" is not encoded and cannot be** — that is a judgement about a particular institution's terms and jurisdiction; the module says a feature is not contract-gated, never that anybody cleared it. |
| **158 Contracted mode** `BUILT` | `permits()` in `app/src/lib/readiness.ts`. A contracted feature needs **both** an agreement and a live connection, and neither is enough: paperwork without plumbing is a button that fails for every student who presses it; plumbing without paperwork succeeds quietly against systems nobody authorised. The agreement is checked first so the refusal names the right half — a missing agreement is a conversation, a missing integration is a configuration task. The admin dashboard, communications and analytics need only the agreement, because there is no system of the university's to connect to. |
